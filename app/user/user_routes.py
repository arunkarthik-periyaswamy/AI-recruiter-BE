...
from app.user.user_service import get_user_by_user_id, \
    user_exists_check, create_user, get_user_list
...
def signup():
    try:
        ...
        if not user:
            ...
            create_user(first_name, last_name, email, password, tenant, role)
...
def logged_in_user_status():
    ...
    try:
        ...
        if 'exp' in payload and datetime.utcnow() > datetime.fromtimestamp(payload['exp']):
            raise jwt.ExpiredSignatureError('Token Expired')
        user = get_user_by_user_id(user_id)
        designations = get_question_bank_designation_for_user(user_id)
        return make_response(jsonify({'token': token,
                                      'user_id': user['user_id'],
                                      'email': user['email'],
                                      'first_name': user['first_name'],
                                      'last_name': user['last_name'],
                                      'designations': designations}), 200)
